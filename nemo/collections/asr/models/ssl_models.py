...
from nemo.core.neural_types import AudioSignal, LengthsType, NeuralType, SpectrogramType, VoidType, LabelsType, AcousticEncodedRepresentation
...
class SpeechEncDecSelfSupervisedModel(ModelPT, ASRModuleMixin, FeatExtractMixin):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        signal, signal_len, transcript, transcript_len = batch
        spectrograms, spec_masks, outputs, encoded_len = self.forward(input_signal=signal,
                                                                      input_signal_length=signal_len,
                                                                      targets=transcript, target_lengths=transcript_len)
