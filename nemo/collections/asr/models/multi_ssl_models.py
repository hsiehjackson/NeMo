...
__all__ = ['SpeechEncMultiDecSelfSupervisedModel']


class SpeechEncMultiDecSelfSupervisedModel(ModelPT, ASRModuleMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        for dec_cfg, loss_cfg in zip(self._cfg.decoders, self._cfg.losses):
            self.decoders.append(SpeechEncMultiDecSelfSupervisedModel.from_config_dict(dec_cfg))
            self.losses.append(SpeechEncMultiDecSelfSupervisedModel.from_config_dict(loss_cfg))

            # can i use modulelist or do I need multidecoder class?
        self.decoders = nn.ModuleList(self.decoders)
        self.losses = nn.ModuleList(self.losses)

        self.spec_augmentation = SpeechEncMultiDecSelfSupervisedModel.from_config_dict(self._cfg.spec_augment)
    ...
    def training_step(self, batch, batch_nb):
        ...
        for i, out in enumerate(decoder_outs):
            self.losses[i].set_num_updates(self.trainer.global_step)
            loss_name = self.loss_log_names[i]
            loss_val = self.losses[i](spectrograms=spectrograms, spec_masks=spec_masks, decoder_outputs=out)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        for i, out in enumerate(decoder_outs):
            ...
            if torch.isnan(loss_val).any():
                continue
            val_loss_dict["val_loss_" + loss_name] = loss_val
            total_loss += loss_val * self.loss_alphas[i]

        val_loss_dict['val_loss'] = total_loss
    ...
    def multi_validation_epoch_end(self, outputs, dataloader_idx: int = 0):
        ...
        val_loss_keys = outputs[0].keys()
        for val_loss_key in val_loss_keys:
            logs[val_loss_key] = torch.stack([x[val_loss_key] for x in outputs]).mean()
