...
class RelPositionMultiHeadAttention(MultiHeadAttention):
    ...
    def forward_rel_pos(self, query, key, value, mask, pos_emb, cache=None, cache_next=None):
    ...
    def forward_nooverlap(self, query, key, value, mask, pos_emb, left_chunk_size=None, chunk_size=None,
    ...
    def mask_invalid_locations(self, input_tensor: torch.Tensor, w: int, d: Union[torch.Tensor, int],
                               autoregressive: bool) -> torch.Tensor:
        affected_seq_len, beginning_mask, ending_mask = self._get_invalid_locations_mask(w, d, autoregressive,
                                                                                         input_tensor.device)
    ...
    def forward(self, query, key, value, mask, pos_emb, cache=None, cache_next=None, left_chunk_size=None,
                chunk_size=None, right_chunk_size=None):
        if self.att_type == 'longformer_chunked_rel_pos':
            return self.forward_nooverlap(query, key, value, mask, pos_emb, left_chunk_size=left_chunk_size,
                                          chunk_size=chunk_size, right_chunk_size=right_chunk_size)
        elif self.att_type == 'longformer_overlap_rel_pos':
            return self.forward_overlap(query, key, value, mask, pos_emb, left_context_size=left_chunk_size,
                                        right_context_size=right_chunk_size)
        else:
            assert self.att_type == 'rel_pos'
            return self.forward_rel_pos(query, key, value, mask, pos_emb, cache=cache, cache_next=cache_next)
...
class ChunkedRelPositionalEncoding(PositionalEncoding):
    ...
    def extend_pe(self, length, device):
        ...
        positions = torch.arange(max_left_context_size, -max_right_context_size - 1, -1, dtype=torch.float32,
                                 device=device).unsqueeze(1)
        self.create_pe(positions=positions)
        self.center_pos = torch.tensor(max_left_context_size, dtype=torch.int32, device=device)

    def forward(self, x, left_chunk_size=None, chunk_size=None, right_chunk_size=None, cache_len=0):
        """Compute positional encoding.
        Args:
            x (torch.Tensor): Input. Its shape is (batch, time, feature_size)
        Returns:
