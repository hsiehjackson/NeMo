...
class ASRModuleMixin(ASRAdapterModelMixin):
    ...
    def change_conf_layers(
        self, new_kernel_size, new_ss_channels=None, new_ss_factor=None, update_config: bool = True
    ):
        if not hasattr(self, 'encoder'):
            logging.info(
                "Could not change the conv in encoder "
                "since the model provided does not contain an `encoder` module in its config."
            )
            return
        ...
        self.encoder.change_conf_layers(new_kernel_size, new_ss_channels, new_ss_factor, self.device)
        if update_config:
            self.cfg.encoder.conv_kernel_size = new_kernel_size
