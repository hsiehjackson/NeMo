...
class SpectrogramLogCallback(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        wandb.log({
            ...
            "train_spec_recon": [self.get_image(x, m) for x, m in
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        log_probs = outputs['log_probs']
        spectrograms, masked_spectrograms, spec_masks, spec_recon = outputs['extra']
        spectrograms = spectrograms + spec_masks * 0.1 * (spectrograms.max() - spectrograms.min())
        spec_recon = spec_recon + spec_masks * 0.1 * (spec_recon.max() - spec_recon.min())
