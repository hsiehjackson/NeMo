...
1. google/long-t5-tglobal-base
2. google/long-t5-tglobal-large
3. google/long-t5-tglobal-xl
4. google/long-t5-local-base
5. google/long-t5-local-large
...
def convert_weights(hf_model, nemo_state_dict_path):
    ...
    hf_model = LongT5ForConditionalGeneration.from_pretrained(hf_model, low_cpu_mem_usage=True, torch_dtype=torch_dtype)
    ...
    for k, v in hf_weights.items():
        ...
        ################# RPE Weights ###################
        #################################################
        elif k == 'encoder.block.0.layer.0.LocalSelfAttention.relative_attention_bias.weight':
            nemo_weights['enc_dec_model.encoder_relative_position_embedding.local_position_embedding.weight'] = v
            print(
                f'Mapped {k} to enc_dec_model.encoder_relative_position_embedding.local_position_embedding.weight'
            )
            
        elif k == 'encoder.block.0.layer.0.TransientGlobalSelfAttention.relative_attention_bias.weight':
            nemo_weights['enc_dec_model.encoder_relative_position_embedding.local_position_embedding.weight'] = v
            print(
                f'Mapped {k} to enc_dec_model.encoder_relative_position_embedding.local_position_embedding.weight'
            )
    
        elif k == 'encoder.block.0.layer.0.TransientGlobalSelfAttention.global_relative_attention_bias.weight':
            nemo_weights['enc_dec_model.encoder_relative_position_embedding.global_position_embedding.weight'] = v
            print(
                f'Mapped {k} to enc_dec_model.encoder_relative_position_embedding.global_position_embedding.weight'
            )
        ...
        #################################################
            ...
            if 'final' in k:
                ...
                model_type, block_number, layer_number = _get_model_type_block_layer(k)
                if 'global_input_layer_norm' in k:
                    nemo_weights[
                        f'enc_dec_model.enc_dec_model.{model_type}.model.layers.{block_number}.self_attention.global_norm.weight'
                    ] = v
                    print(
                        f'Mapped {k} to enc_dec_model.enc_dec_model.{model_type}.model.layers.{block_number}.self_attention.global_norm.weight'
                    )                    
                
                elif layer_number == 0 and model_type == 'encoder':
                    nemo_weights[
...
def package_into_nemo_file(
    ...
    with open_dict(base_cfg):
        ...
        base_cfg.encoder.global_block_size = hf_model_config.global_block_size
        base_cfg.encoder.local_context = hf_model_config.local_radius + 1
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with open(nemo_file_path.replace('.nemo', '.yaml'), "w") as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--base_yaml_config",
        type=str,
        default="hf_longt5_base_config.yaml",
